...
it("has a router handler listening for requests", async () => {
  const res = await request(server)
    .put("/v1/comments/4325903459034/moderate")
    .send({});
...
it("returns 200 when the request is successful", async () => {
  ...
  const { moderated, id: commentID, moderatedContent } = res.body;

  expect(moderated).toBe(true);
  expect(commentID).toBe(id);
  expect(moderatedContent).toBe(requestBodyModerated.content);
});
